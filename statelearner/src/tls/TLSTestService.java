...
import tls.messages.Alert;
import tls.messages.Certificate;
import tls.messages.CertificateRequest;
import tls.messages.CertificateVerify;
import tls.messages.ClientHello;
import tls.messages.ClientKeyExchange;
import tls.messages.Finished;
import tls.messages.HandshakeMsg;
import tls.messages.ServerHello;
import tls.messages.ServerHelloDone;
import tls.messages.ServerKeyExchange;
...
//	TLS currentTLS = new TLS10();
	...
	// Special mode to handle Hostapd TLS client
	boolean HOSTAPD = false;
	...
	public void start() throws Exception {
		// We also want to send Server messages as a client,
		// so always load the keys.
		loadServerKey();
	...
	public void reset() throws Exception {
		//System.out.println("RESET");
                if (serverCertificate == null) loadServerKey();
	...
	public String receiveMessages() throws Exception {
		...
		while (input.available() > 0) {
			...
			boolean is_hostapd_alert = HOSTAPD && record.getContentType() == TLS.CONTENT_TYPE_ALERT &&
									   record.getLength() == 2;
			
			if (!is_hostapd_alert && ccs_in) {
				try {
	...
	public String sendChangeCipherSpec() throws Exception {
		...
		/**
		 * Against Hostap only update the keys after sending the first CCS frame.
		 * This is because the second CCS will be ignored, and if we update keys
		 * then Hostap would not return Alert2.20 "bad record MAC".
		 */
		if (!(HOSTAPD && ccs_out))
		{
