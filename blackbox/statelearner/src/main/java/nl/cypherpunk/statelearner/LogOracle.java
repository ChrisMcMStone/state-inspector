	...
	public Word<D> answerQueryCombined(Word<I> prefix, Word<I> suffix) {
		...
		try {
			...
			if (query.length() != response.length()) {
				throw new RuntimeException("Received number of output symbols not equal to number of input symbols ("
						+ query.length() + " input symbols vs " + response.length() + " output symbols)");
			}

			responsePrefix = response.subWord(0, prefix.length());
			responseSuffix = response.subWord(prefix.length(), response.length());

			logger.logQuery("[" + prefix.toString() + " | " + suffix.toString() + " / " + responsePrefix.toString()
					+ " | " + responseSuffix.toString() + "]");
		} finally {
			sul.post();
		}
