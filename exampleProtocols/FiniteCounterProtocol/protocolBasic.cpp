...
int debug = 1;
int counterMax = 8;
int counterTheshold = counterMax;
        ...
        case start:
            if (strncmp(buffer, "INIT", 4) == 0)
            {
                ...
                *instanceState = inited;
                if (debug)
                {
                    printf("Process %d (%d): STATE: start ->init\n", pid, *messageCounter);
                }
            }
            else
            {
                // This is the important new state transition that depends on the messageCounter
                if (*messageCounter >= counterTheshold)
                {
                    send(socketConnection, "ACK\n", strlen("ACK\n"), 0);
                    *instanceState = inited;
                    if (debug)
                ...
                else
                {
                    ...
                    if (debug)
                    {
                        printf("Process %d (%d): STATE: start -> start\n", pid, *messageCounter);
                    }
                }
            }
            break;
        case inited:
            if (strncmp(buffer, "AUTH", 4) == 0)
            {
                ...
                *instanceState = authed;
                if (debug)
                {
                    printf("Process %d (%d): STATE: inited -> authed\n", pid, *messageCounter);
                }
            }
            else if (strncmp(buffer, "INIT", 4) == 0)
            {
                ...
                if (debug)
                {
                    printf("Process %d (%d): STATE: inited -> inited\n", pid, *messageCounter);
                }
            }
            else
            {
                ...
                *instanceState = error;
                if (debug)
                {
                    printf("Process %d (%d): STATE: inited -> error\n", pid, *messageCounter);
                }
            }
            break;
        case authed:
            if (strncmp(buffer, "DATA", 4) == 0 || strncmp(buffer, "AUTH", 4) == 0)
            {
                ...
                *instanceState = authed;
                if (debug)
                {
                    printf("Process %d (%d): STATE: authed -> authed\n", pid, *messageCounter);
                }
            }
            ...
            else
            {
                ...
                *instanceState = error;
                if (debug)
                {
                    printf("Process %d (%d): STATE: authed -> error\n", pid, *messageCounter);
                }
            }
            break;
        case error:
            ...
            else
            {
                ...
                if (debug)
                {
                    printf("Process %d (%d): STATE: error -> error\n", pid, *messageCounter);
                }
            }
            break;
        }
